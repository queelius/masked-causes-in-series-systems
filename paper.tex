...
Our central contribution is a rank-based identifiability theory for the
masked-cause model: identifiability holds when an augmented candidate-set
matrix has full column rank, and diagnostically confounded components
(those that always co-occur in the candidate set) cannot be separately
identified.
We show that mere separability of the candidate-set structure is
\emph{not} sufficient for identifiability, and we give a confounding-graph
decomposition that characterizes exactly which parameter combinations
(the aggregate hazards of the graph's connected super-components) remain
estimable when full identifiability fails.
...
results:
\emph{(i)}~a formal identifiability theory with two parts: a checkable
sufficient condition (full column rank of the augmented candidate-set
matrix~$\tilde{C}$, Theorem~\ref{thm:identifiability}) for recovering
component parameters from masked data, and a confounding-graph
decomposition (Theorem~\ref{thm:partial-identifiability}) that, when
that rank condition fails, characterizes exactly which parameter
combinations remain estimable;
...
technical advance.
Its first part turns recoverability of the component parameters into a
checkable rank condition on the candidate-set structure; a perhaps
surprising consequence (Remark~\ref{rem:linear-independence}) is that
candidate-set \emph{separability}, the natural pairwise condition, is
\emph{not} sufficient for identifiability, so the rank condition
on~$\tilde{C}$ cannot be replaced by separability alone.
Its second part, the partial-identifiability decomposition
(Theorem~\ref{thm:partial-identifiability}), gives a graph-theoretic
account of exactly which aggregate parameters remain estimable when
full identifiability fails, a refinement with no counterpart in the
classical exact-cause results of \citet{meilijson1981}
and~\citet{nowik1990}.
...
The candidate-set structure central to this paper also appears, under
different names, outside reliability: in weakly supervised learning,
each instance carries a set of candidate labels exactly one of which is
correct, as in the partial-label formulation of \citet{cour2011partial}
and the superset-learning view of \citet{huellermeier2015superset}.
Those works minimize a surrogate classification loss for predictive
accuracy, whereas the present paper poses a parametric identifiability
question (when does the augmented candidate-set matrix~$\tilde{C}$
recover the component parameters) for the cause-of-failure model;
the broader claim that the rank condition is a single cross-domain
criterion unifying masked reliability with weak supervision is
developed separately~\citep{towell2026weaksupcoarsening} and is not
argued here.
...
The symmetry assumption is standard in the masked competing-risks
literature, where it underlies most analyses surveyed by
\citet{sen2001masked}; \citet{yu2010conditional} give a probabilistic
interpretation of it in terms of an auxiliary masking variable and note
that it restricts the admissible class of masking distributions.
Our treatment differs in role rather than in form: rather than positing
symmetry as a modeling convenience, we isolate it as one of the three
conditions (C1--C2--C3) under which the masking factor separates from
the score, and Theorem~\ref{thm:identifiability} makes explicit what its
candidate-set structure must satisfy for the surviving likelihood to
identify~$\btheta$.
...
the incidence matrix of minimal fatal sets, and \citet{nowik1990}
sharpened this to a necessary-and-sufficient condition on the
system's cut-set structure (identifiability holds if and only if at
most one component lies in all cut sets).
Both results assume the exact failed component is recorded at autopsy;
Theorem~\ref{thm:identifiability} is the masked-data instance of this
rank criterion, replacing the observed exact cause with the augmented
candidate-set matrix~$\tilde{C}$ when the diagnostic reports only a
candidate set, and Theorem~\ref{thm:partial-identifiability} adds the
confounding-graph decomposition of partial identifiability, which has
no counterpart in the exact-cause autopsy model.
...
\begin{itemize}
    ...
    \item A rank-based identifiability theorem: the parameter vector is
    identifiable when the augmented candidate-set matrix has full column
    rank, while diagnostically confounded components are non-identifiable
    (Theorem~\ref{thm:identifiability}); and the observation that
    separability of the candidate-set structure is not by itself
    sufficient (Remark~\ref{rem:linear-independence});
    \item A confounding-graph decomposition of partial identifiability
    (Theorem~\ref{thm:partial-identifiability}) that characterizes the
    aggregate hazards of the graph's super-components as the estimable
    parameter combinations when full identifiability fails;
