...
\citet{towell2025exponential-series}.
\end{remark}
...
Full column rank of~$\tilde{C}$ implies separability
...
space spanned by $a = (1, -1, -1, 1)^\top$. Separability alone is
therefore \emph{not} sufficient for identifiability in general.

Whether separability suffices depends on the per-component
perturbation directions $g_j$, not on the linear (in)dependence of
the hazards $\{h_j\}$. Non-identifiability occurs precisely when a
nonzero $g(t)$ stays in $\operatorname{null}(\tilde{C})$ for all
$t$; in the example above this requires $g(t) = \lambda(t)\,a$ for a
scalar function $\lambda$. A sufficiently rich per-component family
can realize such a confined $g$ with a nonzero parameter difference,
so the mechanism is non-identifiable despite separability and
despite the hazards being linearly independent as a set. The
condition that genuinely rescues separability is functional
confinement of the perturbations: e.g. scale-only perturbation of
Weibull components with \emph{distinct fixed shapes} $k_j$ gives
$g_j(t) = \delta_j t^{k_j-1}$, whose distinct exponents cannot align
to a common direction $\lambda(t)\,a_j$ unless every $\delta_j = 0$.
For families with linearly dependent hazards (e.g., exponential
components, or homogeneous Weibull with a common shape), full column
