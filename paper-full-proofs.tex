...
\emph{Step~3: Component-wise equality via linear independence.}
...
all-ones row from~\eqref{eq:total-hazard-eq}.
Steps~1 and~2 give $\tilde{C} \, g(t) = 0$ for all $t > 0$,
where $g(t) = (g_1(t), \ldots, g_m(t))^\top$.
If $\tilde{C}$ has full column rank~$m$, then $g(t) = 0$ for all~$t$
and we are done.
In general, suppose $g(t) \neq 0$ for some~$t$; then $g(t)$ is a
nonzero vector in $\operatorname{null}(\tilde{C})$.
For any $a = (a_1, \ldots, a_m)^\top \in \operatorname{null}(\tilde{C})$,
the rows of~$\tilde{C}$ give $\sum_{j \in c} a_j = 0$ for all
$c \in \mathcal{S}$ and $\sum_j a_j = 0$.
But from~\eqref{eq:candidate-eq}, $\sum_{j \in c} h_j(t) =
\sum_{j \in c} h_j'(t)$ for every~$c$, so in particular
$\sum_j a_j h_j(t) = \sum_j a_j h_j'(t)$.
Since $a_j g_j(t) = a_j h_j(t) - a_j h_j'(t)$, summing gives
$\sum_j a_j g_j(t) = 0$---which is already guaranteed by
$a \in \operatorname{null}(\tilde{C})$ and $\tilde{C}g = 0$.
The linear independence hypothesis resolves this:
if $\{h_1, \ldots, h_m\}$ are linearly independent over $(0,\infty)$,
then the constraint equations from distinct candidate sets, combined
with separability (which ensures that for every component~$j$ there
exist $c_1 \ni j$ and $c_2 \not\ni j$ in~$\mathcal{S}$),
generate enough independent functional relations on the~$g_j$ to force
$g_j(t) = 0$ for all~$j$ and~$t$.
Specifically, subtracting~\eqref{eq:candidate-eq} for $c_2$ from
that for $c_1$ eliminates all components in $c_1 \cap c_2$ and yields
a nontrivial linear combination of the~$h_j$ that must vanish;
linear independence then forces each coefficient to be zero.
